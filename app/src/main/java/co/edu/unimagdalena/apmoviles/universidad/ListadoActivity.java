...
public class ListadoActivity extends AppCompatActivity implements SearchView.OnQueryTextListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Cursor c = almacenController.allAlmacen2(); //en esta linea es donde se traen los datos de la base de datos
        adapter = new AlmacenCursorAdapter(this,c,false);//se pasan los datos por el adaptador
        listado.setAdapter(adapter);//se ponen los datos en la lista
        listado.setTextFilterEnabled(true);
        adapter.setFilterQueryProvider(new FilterQueryProvider() {
            @Override
            public Cursor runQuery(CharSequence constraint) {
                return almacenController.filtrarAlmacen(constraint);
            }
        });
